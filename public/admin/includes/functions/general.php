...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    //****estilo para as label para as informações do configurador*******//
    $style_b= "style=\"color:#1E90FF;\"";
 //echo "<pre>"; print_r($address); echo "</pre>";
//*Informações de nr rua e complemento*//
    if (empty($address['email_address'])){
        $nr_rua = $address['nr_rua'];
        if ($nr_rua != '')
            $street2 ='<b '.$style_b.'>Nº :</b> '.tep_output_string_protected($nr_rua);
        else
            $street2 ='<b '.$style_b.'>Nº :</b> Não informado.';
        $comp_ref = $address['comp_ref'];
        if ($comp_ref != '')
            $street3 ='<b '.$style_b.'>Complemento :</b> '.tep_output_string_protected($comp_ref);
        else
            $street3 ='<b '.$style_b.'>Complemento :</b> Não informado.';
    }
    else {
        $endereco_padrao = tep_db_query("SELECT `customers_default_address_id` FROM `customers` WHERE `customers_id` = ".(int)$id_cliente." LIMIT 0, 30 ");
        $default_address = tep_db_fetch_array($endereco_padrao);
        $id_endereco = $default_address['customers_default_address_id'];
        $info_endereco = tep_db_query("SELECT `entry_nr_rua`, `entry_comp_ref` FROM `address_book` WHERE `address_book_id` = ".(int)$id_endereco);
        while ($end_cliente = tep_db_fetch_array($info_endereco)){
            $nr_rua = $end_cliente['entry_nr_rua'];
            $comp_ref = $end_cliente['entry_comp_ref'];
        }
...
//********************fim***********************//
    ...
    if (isset($address['firstname']) && tep_not_null($address['firstname'])) {
      $firstname = '<b '.$style_b.'>Nome:</b> '.tep_output_string_protected($address['firstname']);
      $lastname = tep_output_string_protected($address['lastname']);
    } elseif (isset($address['name']) && tep_not_null($address['name'])) {
      $firstname = '<b '.$style_b.'>Nome:</b> '.tep_output_string_protected($address['name']);
      $lastname = '';
    } else {
      $firstname = '';
      $lastname = '';
    }
    $street = '<b '.$style_b.'>Rua / Av:</b> '.tep_output_string_protected($address['street_address']);
    if (empty($address['suburb'])){
        $suburb = '<b '.$style_b.'>Bairro :</b> Não informado.';
    } else
    $suburb = '<b '.$style_b.'>Bairro :</b> '.tep_output_string_protected($address['suburb']);
    $city = '<b '.$style_b.'>Cidade :</b> '.tep_output_string_protected($address['city']);
    $state = '<b '.$style_b.'>Estado :</b> '.tep_output_string_protected($address['state']);
    if (isset($address['country_id']) && tep_not_null($address['country_id'])) {
      $country = '<b '.$style_b.'>País :</b> '.tep_get_country_name($address['country_id']);

      if (isset($address['zone_id']) && tep_not_null($address['zone_id'])) {
        $state = tep_get_zone_code($address['country_id'], $address['zone_id'], $state);
      }
    } elseif (isset($address['country']) && tep_not_null($address['country'])) {
      $country = '<b '.$style_b.'>País :</b> '.tep_output_string_protected($address['country']);
    } else {
      $country = '';
    }
    $postcode = '<b '.$style_b.'>CEP :</b> '.tep_output_string_protected($address['postcode']);
    $zip = $postcode;
...
// Text Mode
    ...
    $statecomma = '';
$streets = $street;
$streets = $street . $cr . $street2 . $cr . $street3;
