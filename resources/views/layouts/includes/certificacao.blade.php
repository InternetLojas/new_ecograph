<h1 class="text-center text-danger"><span style="padding:2px;background-color: #808080">Certificações Ecograph</span></h1>
<div class="row">
    <div class="col-md-8">
        <h1 class="text-success">compromisso com o meio ambiente</h1>  
        <div class="row">
            <div class="col-md-4">
                <img src="images/empresa/fsc-ecograph.png" class="center-block img-responsive">
            </div>
            <div class="col-md-8">
                <p class="lead text-right">
                    Compromisso com o meio ambiente e responsabilidade a Ecograph adquiriu o selo <span class="text-success">FSC® (Forest Stewardship Council)</span>.<br>
                    A certificação garante que seus produtos procede de madeiras de floresta com manejo economicamente viável e ambientalmente adequado. 
                </p>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <img src="images/empresa/planta-ecograph.png" class="img-responsive">
    </div>
</div>